...
for subject in os.listdir(os.getcwd()):
	if subject.startswith(('C','F','N')):
		ts_directory = '{subject}/timeSeries'.format(subject=subject)
        	ds_rate = '5ds', '4ds', '3ds'
        	for ds in ds_rate:
			wb_txt = '{subject}/timeSeries/{ds}_filtered_func_showall.txt'.format(subject=subject, ds=ds)
            		thal_txt = '{subject}/timeSeries/{ds}_B_thalamus_masked_showall.txt'.format(subject=subject, ds=ds)
            		...
            		a.to_csv(os.path.join(ts_directory, 'edit_wb_{ds}_ts.csv').format(ds=ds), index=False, index_label=False)
            		b.to_csv(os.path.join(ts_directory, 'edit_thal_{ds}_ts.csv').format(ds=ds), index=False, index_label=False)
